...
// import { Accounts } from 'meteor/accounts-base'; components
...
class Login extends Component {
  ...
  render() {
    ...
    const {loginWithPassword, classes} = this.props;

    function isErrorMessageShowUp(error) {
      return (
        ...
        <Card className={classes.card}>
          ...
          <CardActions className={classes.center}>
            <Button
              raised
              color="primary"
              onClick={() => this.handleLogin(email, password, loginWithPassword)}>Login</Button>
          </CardActions>
          <Typography type="body2">
            <Link to="/signin">Create Account ?</Link>
          </Typography>
          {error !== ''
            ? isErrorMessageShowUp(error)
            : undefined}
        </Card>
      </div>
    );
